...
const initial = read("src/layouts/BaseLayout.astro").match(
  /<script is:inline>([\s\S]*?)<\/script>/,
)[1];
const interaction = ts.transpile(
  read("src/components/ThemeToggle.astro").match(
    /<script>([\s\S]*?)<\/script>/,
  )[1],
...
function themeHarness({ dark = false, saved = null, blocked = false } = {}) {
  ...
  const select = {
    ...
    addEventListener: (name, handler) => {
      controlEvents[name] = handler;
    },
  };
  const media = {
    matches: dark,
    addEventListener: (name, handler) => {
      mediaEvents[name] = handler;
    },
  };
  const storage = {
    getItem: () => {
  ...
  return {
    ...
    storage(value) {
      events.storage({ key: "theme", newValue: value });
    },
  };
}
...
test("production pages preserve all internal destinations, anchors and content URLs", () => {
  ...
  for (const file of files) {
    ...
    for (const [, href] of html.matchAll(/href="([^"]+)"/g)) {
      ...
      const url = new URL(
        href.replaceAll("&amp;", "&"),
        "https://example.test" + file.slice(dist.length).replaceAll("\\", "/"),
      );
      const dest = href.startsWith("#")
        ? file
        : resolve(dist, "." + decodeURIComponent(url.pathname));
  ...
  for (const name of readdirSync(resolve(root, "src/content/blog"))) {
    ...
    assert.ok(
      existsSync(
        resolve(dist, "blog", name.replace(/\.mdx?$/, ""), "index.html"),
      ),
    );
  }
...
test("tag filtering preserves commas and combines category with tag", () => {
  const source = ts.transpile(
    read("src/components/ListGrid.astro")
  ...
  const cards = [
    {
      dataset: { tags: JSON.stringify(["a,b"]), category: "Tech" },
      hidden: false,
    },
    {
      dataset: { tags: JSON.stringify(["a", "b"]), category: "Tech" },
    ...
    {
      dataset: { tags: JSON.stringify(["a,b"]), category: "Thoughts" },
      hidden: false,
    },
  ];
  const form = {
    ...
    querySelector: (selector) =>
      selector.includes("category") ? category : tag,
    addEventListener: (name, fn) => {
      handlers[name] = fn;
    },
  };
  const root = {
    querySelector: (selector) =>
      ({
        "[data-filters]": form,
        "[data-visible-count]": count,
        "[data-tag-empty]": empty,
      })[selector],
    querySelectorAll: (selector) =>
      selector === "[data-entry-card]" ? cards : [],
  };
  ...
  runInNewContext(source, {
    ...
    onPage: (init) => init(new AbortController().signal, () => {}),
    location,
    URL,
    URLSearchParams,
    history: {
